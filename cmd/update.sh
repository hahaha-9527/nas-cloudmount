...
if docker compose -p {{service}} ps &>/dev/null; then
    docker compose -p {{service}} down
else
    echo "Project '{{service}}' not found, skipping..."
fi

# 升级会重建容器，挂载点必须干净，否则新容器把挂载建在死挂载之上
wait_unmount

mkdir -p bak/cmd
cp -f docker-compose.yaml ./bak/ 2>/dev/null || true
cp -f icon.png ./bak/ 2>/dev/null || true
cp -f config.json ./bak/ 2>/dev/null || true
cp -f cmd/install.sh ./bak/cmd/install.sh 2>/dev/null || true
cp -f cmd/uninstall.sh ./bak/cmd/uninstall.sh 2>/dev/null || true

if [ -d update_file ]; then
    cp -f update_file/icon.png ./ 2>/dev/null || true
    cp -f update_file/config.json ./ 2>/dev/null || true
    cp -r -f update_file/cmd/install.sh ./cmd/install.sh 2>/dev/null || true
    cp -r -f update_file/cmd/uninstall.sh ./cmd/uninstall.sh 2>/dev/null || true
    rm -rf update_file
fi
...
echo "Update completed. Waiting compose up ..."
