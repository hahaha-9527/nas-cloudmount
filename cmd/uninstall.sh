...
# 配置面板（宿主上的 systemd 服务）先停掉并移除 unit，否则卸载后 8791 上
# 还留着一个能重启已删容器的页面。
# 数据目录 /volume1/data/cloudmount 保留：挂载配置、口令记录、读缓存都在那里。
bash ./cmd/panel.sh remove || true
...
# 兜底：换过部署渠道的容器不在本项目名下，compose down 删不掉它，会一直占着名字
...
# compose down 正常会连挂载一起卸载并传播回来；被强杀时靠这里兜底
wait_unmount

# 挂载点目录清掉（只在本应用挂过、且已空的目录上生效；rmdir 对非空目录无害）
# 留着它会在「我的文件」里显示成一个空文件夹，容易让人以为数据丢了。
if [ -d "${MOUNTPATH}" ] && ! is_mounted; then
    rmdir "${MOUNTPATH}" 2>/dev/null && echo "  removed empty mount dir ${MOUNTPATH}" || true
fi

# rclone/rclone 为公共镜像，不删（重装时省一次下载）
echo "Cache at /volume1/data/cloudmount is kept."
